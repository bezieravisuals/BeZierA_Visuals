...
const mailTransport = nodemailer.createTransport({
  ...
  secure: false,
  auth:
    gmailPass && gmailUser ? { user: gmailUser, pass: gmailPass } : undefined,
});
app.use(cors());
...
const adminEmail = process.env.ADMIN_EMAIL || "admin@beziera.com";
const adminPassword = process.env.ADMIN_PASSWORD || "change-this-password";

const ensureDefaultAdmin = () => {
  if (!adminPassword || adminPassword === "change-this-password") {
    console.warn("⚠️  WARNING: Admin password is not configured. Set ADMIN_PASSWORD in .env");
  }
  
  const existingAdmin = users.find(
    (user) => user.email === adminEmail || user.username === adminEmail,
  );
...
app.get("/api/health", (_, res) =>
  res.json({
    status: "ok",
    database: process.env.MONGODB_URI ? "configured" : "memory-fallback",
  }),
);
  ...
  try {
    await mailTransport.sendMail({
      ...
      text: welcomeMessage
        ...
        .replace(/^- /gm, "")
        .replace(/\n\n/g, "\n"),
      html: `<div style="font-family: Arial, sans-serif; line-height: 1.7; color: #111827;"> <p>Hi <strong>${username}</strong>,</p> <p>Welcome to <strong>BeZierA Visuals!</strong> 🎉</p> <p>We're thrilled to have you join our creative community, where ideas are transformed into stunning visuals, powerful digital experiences, and innovative solutions.</p> <h3>Your Account is Ready</h3> <p>Your BeZierA account has been successfully created, and you can now access your personalized dashboard to:</p> <ul> <li>Submit your creative project ideas.</li> <li>Track your project status in real time.</li> <li>Communicate with our creative team.</li> <li>Receive project updates and completed work.</li> <li>Explore our premium portfolio.</li> </ul> <h3>Our Services</h3> <ul> <li>Graphic Designing</li> <li>3D Animation & Modeling</li> <li>Full-Stack Development</li> <li>Video Editing</li> </ul> <p><strong>"Your Vision. Our Creativity. Beyond Imagination."</strong></p> <p>Every project is crafted with creativity, precision, and professionalism to bring your vision to life.</p> <h3>Need Help?</h3> <p>We're always here to assist you.</p> <p><strong>Contact Us</strong></p> <ul> <li>📞 Phone: 9443104878</li> <li>📧 Email: bezieravisuals@gmail.com</li> <li>💬 WhatsApp: 9443104878</li> <li>📷 Instagram: beziera_visuals</li> <li>💼 LinkedIn: BeZierA Visuals</li> </ul> <p>Thank you for choosing <strong>BeZierA Visuals</strong>.</p> <p>We look forward to creating something extraordinary together.</p> <p>Warm regards,<br /><strong>The BeZierA Visuals Team</strong></p> <p><em>Your Vision. Our Creativity. Beyond Imagination.</em></p> <p>© 2026 BeZierA Visuals. All Rights Reserved.</p> </div>`,
    });
  } catch (error) {
...
app.post("/api/auth/login", async (req, res) => {
  const user = users.find(
    (item) =>
      item.email === req.body.identifier ||
      item.username === req.body.identifier,
  );
  if (
    !user ||
...
app.get("/api/me", auth, (req, res) =>
  res.json(users.find((user) => user.id === req.user.id)),
);
...
app.delete("/api/projects/:id", auth, (req, res) => {
  ...
  if (!allowedRole || (req.user.role !== "Main Admin" && project.status !== "Completed")) {
    return res
      ...
      .json({
        message:
          "Only the admin can remove active projects. Managers can remove completed projects.",
      });
  }
  ...
  try {
    if (!gmailPass || gmailPass === "your_google_app_password_here") {
      return res
        ...
        .json({
          message:
            "Gmail SMTP is not configured. Add a valid Google App Password to MAIL_PASS in .env.",
        });
    }
    const recipient = process.env.MAIL_TO || "bezieravisuals@gmail.com";
    const mailFrom =
      process.env.MAIL_FROM || "BeZierA Contact <bezieravisuals@gmail.com>";
    await mailTransport.sendMail({
      from: mailFrom,
      to: recipient,
    ...
    res
      .status(500)
      .json({
        message: "Email delivery failed. Check the Gmail App Password in .env.",
      });
  }
});
app.get(
  "/api/admin/analytics",
  auth,
  role("Main Admin", "Department Manager"),
  (_, res) =>
    res.json({
      totals: {
        users: users.length,
...
app.post("/api/send-welcome-email", async (req, res) => {
  ...
  const welcomeMessage = `Hi ${name || "there"},\n\nWelcome to **BeZierA Visuals!** 🎉\n\nWe're thrilled to have you join our creative community, where ideas are transformed into stunning visuals, powerful digital experiences, and innovative solutions.\n\n## Our Services\n\n- Graphic Designing\n- 3D Animation & Modeling\n- Full-Stack Development\n- Video Editing\n\n## Our Promise\n**"Your Vision. Our Creativity. Beyond Imagination."**\n\nEvery project is crafted with creativity, precision, and professionalism to bring your vision to life.\n\n## Need Help?\nWe're always here to assist you.\n\n**Contact Us**\n\n- 📞 Phone: 9443104878\n- 📧 Email: bezieravisuals@gmail.com\n- 💬 WhatsApp: 9443104878\n- 📷 Instagram: beziera_visuals\n- 💼 LinkedIn: BeZierA Visuals\n\nThank you for choosing **BeZierA Visuals**.\n\nWarm regards,\n\n**The BeZierA Visuals Team**\n\n*Your Vision. Our Creativity. Beyond Imagination.*\n\n© 2026 BeZierA Visuals. All Rights Reserved.`;
  try {
    await mailTransport.sendMail({
      from: process.env.MAIL_FROM || "BeZierA Visuals <bezieravisuals@gmail.com>",
      to: email,
      replyTo: "bezieravisuals@gmail.com",
      subject: `Welcome to BeZierA Visuals, ${name || "Friend"}!`,
      text: welcomeMessage
        .replace(/\*\*/g, "")
        .replace(/## /g, "")
        .replace(/^- /gm, "")
        .replace(/\n\n/g, "\n"),
      html: `<div style="font-family: Arial, sans-serif; line-height: 1.7; color: #111827;"> <p>Hi <strong>${name || "there"}</strong>,</p> <p>Welcome to <strong>BeZierA Visuals!</strong> 🎉</p> <p>We're thrilled to have you join our creative community, where ideas are transformed into stunning visuals, powerful digital experiences, and innovative solutions.</p> <h3>Our Services</h3> <ul> <li>Graphic Designing</li> <li>3D Animation & Modeling</li> <li>Full-Stack Development</li> <li>Video Editing</li> </ul> <p><strong>"Your Vision. Our Creativity. Beyond Imagination."</strong></p> <p>Every project is crafted with creativity, precision, and professionalism to bring your vision to life.</p> <h3>Need Help?</h3> <p>We're always here to assist you.</p> <p><strong>Contact Us</strong></p> <ul> <li>📞 Phone: 9443104878</li> <li>📧 Email: bezieravisuals@gmail.com</li> <li>💬 WhatsApp: 9443104878</li> <li>📷 Instagram: beziera_visuals</li> <li>💼 LinkedIn: BeZierA Visuals</li> </ul> <p>Thank you for choosing <strong>BeZierA Visuals</strong>.</p> <p>Warm regards,<br /><strong>The BeZierA Visuals Team</strong></p> <p><em>Your Vision. Our Creativity. Beyond Imagination.</em></p> <p>© 2026 BeZierA Visuals. All Rights Reserved.</p> </div>`,
    });
...
app.listen(port, () =>
  console.log(`BeZierA API running at http://localhost:${port}`),
);
